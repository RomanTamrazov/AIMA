...
class TelegramBot:
    def __init__(self):
        ...
        self.user_context = {}
        self.user_profiles = {}
        self.pending_approvals = {}  # Словарь для хранения заявок на согласование
        self.managers_list = {}  # Список руководителей для отправки уведомлений
    ...
    def _is_working_hours(self):
        """Проверяет, рабочее ли сейчас время (пн-пт 9:00-18:00)"""
        now = datetime.now()
    ...
    def _get_manager_for_employee(self, employee_id):
        """Находит руководителя для сотрудника"""
        for user_id, profile in self.user_profiles.items():
            if profile['role'] == 'manager':
                return user_id
        return None
    
    async def _send_approval_request(self, context: CallbackContext, employee_id: int, event: dict):
        """Отправляет заявку на согласование руководителю"""
        manager_id = self._get_manager_for_employee(employee_id)
        if not manager_id:
            return False
        
        # Сохраняем заявку в ожидании
        approval_id = f"{employee_id}_{event['title']}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        # Если не рабочее время, сохраняем заявку для отправки позже
        if not self._is_working_hours():
            return True
        
        # Отправляем уведомление руководителю
        try:
            employee_name = f"Сотрудник {employee_id}"
            event_info = f"""
📋 Новая заявка на согласование мероприятия
...
🎪 Тип: {event.get('type', 'мероприятие')}
            ...
            print(f"❌ Ошибка отправки заявки руководителю: {e}")
            return False
    
    async def _handle_approval_response(self, update: Update, context: CallbackContext, approval_id: str, approved: bool):
        """Обрабатывает ответ руководителя на заявку"""
        ...
        try:
            await context.bot.send_message(
                chat_id=employee_id,
                text=f"📋 Ваша заявка на мероприятие '{event['title']}' {status_text} руководителем"
            )
        except Exception as e:
            print(f"❌ Ошибка отправки уведомления сотруднику: {e}")
        
        # Обновляем сообщение у руководителя
        response_text = "✅ Заявка одобрена" if approved else "❌ Заявка отклонена"
...
Выберите локацию и нажмите 🔍 Найти по локации
    ...
    async def _show_event_page(self, update: Update, context: CallbackContext, user_id: int, page: int):
        ...
        if profile['role'] == 'employee':
            action_buttons.append(InlineKeyboardButton("📋 Согласовать с руководителем", callback_data=f"approve_{page}"))
...
"""
    ...
    async def handle_callback(self, update: Update, context: CallbackContext):
        ...
        try:
            if data.startswith('approve_') and not data.startswith('approve_') and len(data.split('_')) == 2:
                ...
                if events and page < len(events):
                    ...
                    if profile['role'] == 'employee':
                        success = await self._send_approval_request(context, user_id, event)
                        if success:
                            if self._is_working_hours():
                                await query.answer("📋 Заявка отправлена руководителю")
                            else:
                                await query.answer("📋 Заявка сохранена, будет отправлена в рабочее время")
                        else:
                            await query.answer("❌ Ошибка отправки заявки")
                        return
            
            elif data.startswith('approve_') and len(data.split('_')) > 2:
                approval_id = data.replace('approve_', '')
                await self._handle_approval_response(update, context, approval_id, True)
                return
            
            elif data.startswith('reject_'):
                approval_id = data.replace('reject_', '')
                await self._handle_approval_response(update, context, approval_id, False)
                return
                ...
                if events and page < len(events):
                    ...
                    if profile['role'] == 'employee':
                        ...
                        for approval_id, approval in self.pending_approvals.items():
                            if (approval['employee_id'] == user_id and 
                                approval['event']['title'] == event['title'] and 
                        ...
                        if not event_approved:
                            keyboard = [
                                [InlineKeyboardButton("📋 Согласовать с руководителем", callback_data=f"approve_{page}")],
                                [InlineKeyboardButton("🎯 Главное меню", callback_data="main_menu")]
                            ]
                            reply_markup = InlineKeyboardMarkup(keyboard)
                            
                            await query.edit_message_text(
                                "❌ Для добавления мероприятия в календарь необходимо согласование с руководителем\n\n"
...
• 🔍 - Расширенный поиск
• 📊 - Статистика
• 📋 - Согласование с руководителем (для сотрудников)
        """
    ...
    def run(self):
        ...
        print("   • 📋 Система согласования с руководителем")
        print("   • ⏰ Отправка заявок только в рабочее время")
