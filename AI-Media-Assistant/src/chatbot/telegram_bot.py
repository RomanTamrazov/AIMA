...
class TelegramBot:
    ...
    def _get_user_profile(self, user_id):
        # Пробуем найти профиль по user_id как число
        if user_id in self.user_profiles:
            return self.user_profiles[user_id]
        
        # Пробуем найти профиль по user_id как строка
        if str(user_id) in self.user_profiles:
            return self.user_profiles[str(user_id)]
    ...
    def _get_user_auth(self, user_id):
        # Пробуем найти auth по user_id как число
        if user_id in self.user_auth:
            return self.user_auth[user_id]
        
        # Пробуем найти auth по user_id как строка
        if str(user_id) in self.user_auth:
            return self.user_auth[str(user_id)]
        
        return {'status': 'unauthorized', 'role': None}
...
Для отмены регистрации нажмите "❌ Отмена"
    ...
    async def handle_auth(self, update: Update, context: CallbackContext):
        ...
        print(f"DEBUG: handle_auth - text: {text}, context: {current_context}, user_id: {user_id} (type: {type(user_id)})")
        
        # ДЕБАГ: Выводим все user_id из системы для проверки
        print(f"DEBUG: All user_ids in auth: {list(self.user_auth.keys())}")
        print(f"DEBUG: All user_ids in profiles: {list(self.user_profiles.keys())}")
        ...
        if current_context == 'auth_menu':
            ...
            if text == "✅ Войти в систему":
                # Проверяем существование пользователя (с учетом разных типов user_id)
                user_found = False
                auth_data = None
                
                # Проверяем user_id как число
                if user_id in self.user_auth:
                    auth_data = self.user_auth[user_id]
                    user_found = True
                # Проверяем user_id как строку (на случай если в JSON хранится как строка)
                elif str(user_id) in self.user_auth:
                    auth_data = self.user_auth[str(user_id)]
                    user_found = True
                
                if user_found and auth_data and auth_data.get('status') == 'authenticated':
                    # Пользователь найден - входим
                    profile = self._get_user_profile(user_id)
                    
                    await update.message.reply_text("✅ Вы успешно вошли в систему!")
                    
                    # Проверяем, завершена ли настройка профиля
                    if not profile['setup_completed']:
                        await self._show_role_selection(update, context)
                    else:
                    ...
                    await update.message.reply_text(
                        "❌ Аккаунт не найден.\n\nУ вас еще нет аккаунта в системе. Пожалуйста, зарегистрируйтесь.",
                ...
                # Обработка любого другого текста как попытки входа
                user_found = False
                auth_data = None
            ...
            elif text.strip() == self.manager_password:
                if user_id in self.pending_registrations:
                    registration_data = self.pending_registrations[user_id]
            ...
            elif text.strip() == self.admin_password:
                if user_id in self.pending_registrations:
                    registration_data = self.pending_registrations[user_id]
            ...
            if text in ["👔 Руководитель", "👨‍💼 Сотрудник"]:
                ...
                # Проверяем аутентификацию перед переходом в главное меню
    ...
    async def _show_login(self, update: Update, context: CallbackContext):
        ...
        login_keyboard = [
            [KeyboardButton("✅ Войти в систему")],
            [KeyboardButton("⬅️ Назад"), KeyboardButton("❌ Отмена")]
        ]
    ...
    Для входа в систему нажмите кнопку "✅ Войти в систему" ниже.
...
Для согласования используйте кнопки ниже:
    ...
    async def _approve_event(self, query, context, employee_id, request_index):
        try:
            ...
            # Временное решение - просто отмечаем как одобренное без добавления в календарь
            if hasattr(self.calendar, 'add_event_to_calendar'):
                result = self.calendar.add_event_to_calendar(event, employee_id)
            else:
                # Если метода нет, создаем фиктивный успешный результат
                result = {'success': True, 'message': f"✅ Мероприятие '{event['title']}' одобрено и добавлено в календарь сотрудника"}
...
Начните с поиска подходящих мероприятий!
    ...
    def get_calendar_events(self, user_id):
        ...
        try:
            # Проверяем, есть ли метод get_user_events в calendar
            if hasattr(self.calendar, 'get_user_events'):
                return self.calendar.get_user_events(user_id)
            else:
                # Если метода нет, возвращаем пустой список или используем альтернативный способ
                print("⚠️ Метод get_user_events не найден в TelegramCalendar")
                return []
        except Exception as e:
    ...
    async def handle_callback(self, update: Update, context: CallbackContext):
        ...
        if data.startswith('event_'):
            ...
            await self.show_pending_approvals_callback(query, context)
...
Начните с поиска подходящих мероприятий!
    ...
    async def handle_message(self, update: Update, context: CallbackContext):
        ...
        print(f"DEBUG: handle_message - text: {text}, context: {current_context}, user_id: {user_id}")
        
        # ПЕРВОЕ: Проверяем, является ли пользователь администратором и обрабатываем админ-команды
        if self._is_admin(user_id):
            # Проверяем, находится ли пользователь в админ-контексте или использует админ-кнопки
            admin_contexts = ['admin_menu', 'user_management', 'user_details', 'assign_manager_select_employee', 
                            'assign_manager_select_manager', 'password_management', 'change_manager_password',
                            'change_admin_password', 'broadcast_menu', 'broadcast_message']
            
            if current_context in admin_contexts or text.startswith(("👤 ", "👔 ", "👑 ", "🎯 ", "❌ ")):
                await self.handle_admin_commands(update, context)
                return
        
        # ВТОРОЕ: Обработка аутентификации
        if current_context in ['auth_menu', 'login', 'registration_fio', 'registration_position', 
                            'registration_role', 'manager_password', 'admin_password']:
            await self.handle_auth(update, context)
            return
        
        # ТРЕТЬЕ: Обработка настройки профиля
        ...
        # ЧЕТВЕРТОЕ: Обработка редактирования профиля
        ...
        # ПЯТОЕ: Обработка сброса профиля
        ...
        # ШЕСТОЕ: Проверка аутентификации
        if not await self._require_auth(update, context):
            return
        
        profile = self._get_user_profile(user_id)
        
        # СЕДЬМОЕ: Проверка завершения настройки профиля
        ...
        # ВОСЬМОЕ: Обработка основных команд меню
        if text == "🎯 Рекомендованные мероприятия":
            ...
            # Если команда не распознана, проверяем админ-команды еще раз
            if self._is_admin(user_id) and text.startswith(("👤 ", "👔 ", "👑 ")):
                await self.handle_admin_commands(update, context)
            else:
                await update.message.reply_text(
...
Подтвердите сброс профиля:
    ...
    async def handle_admin_commands(self, update: Update, context: CallbackContext):
        ...
        print(f"DEBUG: handle_admin_commands CALLED - text: {text}")
        ...
        print(f"DEBUG: handle_admin_commands - text: {text}, context: {current_context}")
        
        # ОБРАБОТКА КНОПКИ "⬅️ НАЗАД В АДМИНКУ" ИЗ ЛЮБОГО КОНТЕКСТА
        ...
        if current_context == 'admin_menu':
        ...
        elif current_context == 'user_details':
            if text == "⬅️ Назад к списку пользователей":
                await self._show_user_management(update, context)
            elif text == "🔄 Обновить информацию":
                selected_user_id = context.user_data.get('selected_user_id')
                selected_user_fio = context.user_data.get('selected_user_fio')
                if selected_user_id and selected_user_fio:
                    await self._show_user_details(update, context, f"👤 {selected_user_fio}")
            elif text.startswith("🎯 Назначить руководителя для "):
                employee_fio = text.replace("🎯 Назначить руководителя для ", "").strip()
                context.user_data['assign_manager_employee_id'] = context.user_data.get('selected_user_id')
                context.user_data['assign_manager_employee_fio'] = employee_fio
                await self._select_manager_for_employee(update, context, f"👤 {employee_fio}")
            elif text.startswith("❌ Удалить руководителя у "):
                employee_fio = text.replace("❌ Удалить руководителя у ", "").strip()
                employee_id = context.user_data.get('selected_user_id')
                
                if str(employee_id) in self.user_managers:
                    manager_id = self.user_managers[str(employee_id)]
                    del self.user_managers[str(employee_id)]
                    
                    # Удаляем из manager_employees
                    if manager_id in self.manager_employees and employee_id in self.manager_employees[manager_id]:
                    ...
                    await update.message.reply_text(
                        ...
                        reply_markup=ReplyKeyboardMarkup([["🔄 Обновить информацию"], ["⬅️ Назад к списку пользователей"]], resize_keyboard=True)
                    )
                else:
                    await update.message.reply_text("❌ У сотрудника нет назначенного руководителя")
            elif text == "⬅️ Назад в админку":
                await self._show_admin_menu(update, context)
        
        elif current_context == 'assign_manager_select_employee':
            print(f"DEBUG: In assign_manager_select_employee context, text: {text}")
            if text == "⬅️ Назад в админку":
                await self._show_admin_menu(update, context)
            elif text.startswith("👤 "):
                print(f"DEBUG: Processing employee selection: {text}")
                await self._select_manager_for_employee(update, context, text)
            else:
                print(f"DEBUG: Unknown text in assign_manager_select_employee: {text}")
                await update.message.reply_text(
                    "Пожалуйста, выберите сотрудника из списка кнопок ниже.",
            ...
            if text == "⬅️ Назад к выбору сотрудника":
                await self._show_assign_manager_menu(update, context)
            elif text.startswith("👔 ") or text == "❌ Удалить текущего руководителя":
                await self._assign_manager_to_employee(update, context, text)
            elif text == "⬅️ Назад в админку":
                await self._show_admin_menu(update, context)
            else:
                # Если текст не распознан, показываем сообщение об ошибке
                await update.message.reply_text(
                    "Пожалуйста, выберите руководителя из списка кнопок ниже.",
                    reply_markup=ReplyKeyboardMarkup([["⬅️ Назад к выбору сотрудника"], ["⬅️ Назад в админку"]], resize_keyboard=True)
                )
            ...
            # Если команда не распознана в админ-контексте, показываем админ-меню
            await self._show_admin_menu(update, context)
    ...
    async def _show_user_management(self, update: Update, context: CallbackContext):
        ...
        for user in users_list[:10]:
            role_icon = "👑" if user['role'] == 'admin' else "👔" if user['role'] == 'manager' else "👨‍💼"
            users_keyboard.append([KeyboardButton(f"{role_icon} {user['fio']} - {user['role']}")])
        
        users_keyboard.extend([
            [KeyboardButton("🎯 Назначить руководителя")],
            [KeyboardButton("🔄 Обновить список")],
            [KeyboardButton("⬅️ Назад в админку")]  # Эта кнопка теперь будет обрабатываться
        ])
    ...
    Список пользователей (первые 10):
        ...
        for i, user in enumerate(users_list[:10], 1):
            ...
            role_icon = "👑" if user['role'] == 'admin' else "👔" if user['role'] == 'manager' else "👨‍💼"
            text += f"\n{i}. {user['fio']} - {user['position']} ({role_icon} {user['role']}){manager_info}"
    ...
    async def _show_assign_manager_menu(self, update: Update, context: CallbackContext):
        ...
        # Получаем список сотрудников (не руководителей и не администраторов)
        employees = []
        for uid, auth in self.user_auth.items():
    ...
    async def _select_manager_for_employee(self, update: Update, context: CallbackContext, employee_info: str):
        ...
        print(f"DEBUG: _select_manager_for_employee - employee_info: {employee_info}")
        
        # Извлекаем ID сотрудника из текста кнопки
        employee_fio = employee_info.replace("👤 ", "").split(" (👔")[0].strip()
        employee_id = None
        
        print(f"DEBUG: Looking for employee with FIO: {employee_fio}")
        ...
        print(f"DEBUG: Found employee_id: {employee_id}")
        ...
        if not managers:
            await update.message.reply_text(
                ...
                reply_markup=ReplyKeyboardMarkup([["⬅️ Назад к выбору сотрудника"]], resize_keyboard=True)
            )
            return
        
        managers_keyboard = []
        for manager in managers[:10]:
            # Подсчитываем количество подчиненных
            employee_count = len(self.manager_employees.get(str(manager['user_id']), []))
            managers_keyboard.append([KeyboardButton(f"👔 {manager['fio']} ({employee_count} подчин.)")])
        
        # Добавляем кнопку удаления руководителя если он уже назначен
        if str(employee_id) in self.user_managers:
            managers_keyboard.append([KeyboardButton("❌ Удалить текущего руководителя")])
        ...
        current_manager_info = ""
        if str(employee_id) in self.user_managers:
            current_manager_id = self.user_managers[str(employee_id)]
            current_manager_profile = self.user_profiles.get(int(current_manager_id), {})
            current_manager_info = f"\n\n👔 <b>Текущий руководитель:</b> {current_manager_profile.get('fio', 'Не указан')}"
        
        text = f"""
    🎯 Назначение руководителя

    Сотрудник: {employee_fio}{current_manager_info}

    Выберите руководителя из списка:

    👔 - руководитель (количество подчиненных)

    Или нажмите "❌ Удалить текущего руководителя" чтобы удалить текущее назначение.
        """
...
После выбора аудитории введите текст сообщения.
    ...
    async def _request_broadcast_message(self, update: Update, context: CallbackContext):
        user_id = update.effective_user.id
        self._set_user_context(user_id, 'broadcast_message')
        
        audience = context.user_data.get('broadcast_audience', 'all')
        audience_text = {
            'all': 'всем пользователям',
            'managers': 'только руководителям',
...
Аудитория: {audience_text.get(audience, 'всем пользователям')}
...
Для отмены нажмите /cancel
    ...
    async def _send_broadcast(self, update: Update, context: CallbackContext, message_text: str, audience: str):
        user_id = update.effective_user.id
        ...
        try:
            ...
            for uid, auth in self.user_auth.items():
                if auth.get('status') == 'authenticated':
                    ...
                    elif audience == 'managers' and auth.get('role') == 'manager':
                        target_users.append(uid)
                    elif audience == 'employees' and auth.get('role') == 'employee':
                        target_users.append(uid)
...
—
<i>Отправлено администратором системы</i>
                        """,
                        parse_mode='HTML'
                    )
                    success_count += 1
    ...
    async def _show_user_details(self, update: Update, context: CallbackContext, user_info: str):
        try:
            # Извлекаем ФИО из текста кнопки
            user_fio = user_info.replace("👑 ", "").replace("👔 ", "").replace("👨‍💼 ", "").split(" - ")[0].strip()
            user_id = None
            
            # Находим пользователя по ФИО
            for uid, profile in self.user_profiles.items():
                if profile.get('fio') == user_fio:
                    user_id = uid
                    break
            ...
            manager_info = ""
            if str(user_id) in self.user_managers:
                manager_id = self.user_managers[str(user_id)]
                manager_profile = self.user_profiles.get(int(manager_id), {})
                manager_info = f"👔 <b>Руководитель:</b> {manager_profile.get('fio', 'Не указан')}"
            else:
                manager_info = "👔 <b>Руководитель:</b> Не назначен"
            
            role_icon = "👑" if auth.get('role') == 'admin' else "👔" if auth.get('role') == 'manager' else "👨‍💼"
            
            text = f"""
    👤 <b>Детальная информация о пользователе</b>

    {role_icon} <b>ФИО:</b> {profile.get('fio', 'Не указано')}
    💼 <b>Должность:</b> {profile.get('position', 'Не указано')}
    🎭 <b>Роль:</b> {auth.get('role', 'Не указана')}
    {manager_info}
    📅 <b>Дата регистрации:</b> {profile.get('registration_date', 'Не указана')}

    <b>Настройки профиля:</b>
    📍 <b>Локация:</b> {profile['preferences']['location_preference'] or 'не указана'}
    👥 <b>Размер мероприятий:</b> {profile['preferences']['audience_preference'] or 'не указан'}
    🎭 <b>Роль участия:</b> {profile['preferences']['participation_role'] or 'не указана'}
    🎯 <b>Интересы:</b> {', '.join(profile['preferences']['interests']) if profile['preferences']['interests'] else 'не указаны'}
            """
            ...
            if auth.get('role') == 'employee':
                if str(user_id) in self.user_managers:
                    management_keyboard.append([KeyboardButton(f"❌ Удалить руководителя у {user_fio}")])
                else:
                    management_keyboard.append([KeyboardButton(f"🎯 Назначить руководителя для {user_fio}")])
            
            management_keyboard.extend([
                [KeyboardButton("🔄 Обновить информацию")],
                [KeyboardButton("⬅️ Назад к списку пользователей")]
            ])
            
            reply_markup = ReplyKeyboardMarkup(management_keyboard, resize_keyboard=True)
            
            await update.message.reply_text(text, reply_markup=reply_markup, parse_mode='HTML')
            self._set_user_context(update.effective_user.id, 'user_details')
            
            # Сохраняем выбранного пользователя в контексте
            context.user_data['selected_user_id'] = user_id
            context.user_data['selected_user_fio'] = user_fio
            
        except Exception as e:
            print(f"❌ Ошибка показа деталей пользователя: {e}")
            await update.message.reply_text("❌ Ошибка при загрузке информации о пользователе")
    
    async def _assign_manager_to_employee(self, update: Update, context: CallbackContext, manager_info: str):
        user_id = update.effective_user.id
        
        if manager_info == "❌ Удалить текущего руководителя":
        ...
        manager_fio = manager_info.replace("👔 ", "").split(" (")[0].strip()
        manager_id = None
